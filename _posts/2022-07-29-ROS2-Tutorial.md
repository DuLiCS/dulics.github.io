...
和刚才的识别不同的是，这里把图像采集和识别作为两个node。架构也很简单，图像采集作为publisher，识别作为subscriber。
...
def main(args=None):                                 # ROS2节点主入口main函数
    ...
    rclpy.shutdown()                                 # 关闭ROS2 Python接口```
```
Publisher的代码运行时会报错。报错截图如下：

![ROS2](/img/ROS31.png)

原因是CvBridge只支持Python2，所以在Python3的环境中需要重新编译。链接在后 https://zhuanlan.zhihu.com/p/407517632
